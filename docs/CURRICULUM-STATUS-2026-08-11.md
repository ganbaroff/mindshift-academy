...
3. ~~**Week 1**~~ **СДЕЛАНО 2026-09-06.** Три мира вместо одного безымянного поля: `windows`
   (ночной дом, 16 окон), `garden` (грядка, 16 лунок), `tiles` (стена в плитке). Кольцо переносов
   как в неделях 2 и 4: s1 кончает на грядке, s2 на плитке, s3 возвращается в дом — и там уже
   фигура из двух частей, которой дом в первой сессии не ставил.

   Главное, что чинилось, — формулировка. Было: 19 задач из 21 открывались одной из двух фраз
   («сделай так, чтобы совпало с картинкой» — 6 раз, «сделай так, чтобы совпало» — 9, плюс две
   пары дословных дублей). Первая неделя курса — то, что ребёнок видит первым, — звучала как одна
   задача, повторённая двадцать раз. Стало: 21 уникальное открытие, каждое называет ситуацию, а
   не действие. Бриф был у 7 задач из 21, стал у всех 21.

   `target` ни одной задачи не изменён, id не тронуты — правился только текст и добавлено `world`,
   поэтому ответы, прогресс детей и контракты движка не двигались.

   Долг по инварианту: **12 → 4 нарушения**, зелёных недель **2 → 3**. Храповик снова потребовал
   внести неделю в `GREEN_WEEKS` тем же PR. Гейт `scripts/good-child-week1.mjs` (запускать через
   `npx tsx`, не `node` — он читает `@/lib`): 21 из 21 задач PASS 3/3, утечки фигуры в promptRu нет.
