...
- **Ни в одной из 81 задач ребёнок не пишет ни слова.** `grep -n "<input|<textarea|contentEditable"`
  по всему `src/components/curriculum` даёт **ноль совпадений**. `GridDrawSurface` — клики по
  клеткам, `RuleSurface` — выпадающие списки (`RuleChoice`, `onChange` на `select`),
  `SequenceSurface` — кнопки, `ClaimSurface` и `PatternSurface` — выбор. Курс, который заявлен
  как обучение точности формулировки, не даёт ребёнку сформулировать ни одной фразы.
...
- **Тир должен менять задачу, а не подсказку.** Минимум: тир 1 — образец на экране; тир 2 —
  образец убран; тир 3 — добавлено ограничение («без двух одинаковых шагов подряд», «правило
  должно пройти и скрытую карту»). Это `low floor / high ceiling` по Resnick: один и тот же
  предмет, разные пути наверх ([Designing for Wide Walls](https://mres.medium.com/designing-for-wide-walls-323bdb4e7277)).
