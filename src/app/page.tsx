...
export default function HomePage() {
  ...
  return (
    <main className="relative isolate min-h-screen bg-background text-foreground">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-[var(--surface-strong)]" />
      ...
      <header className="relative z-10 mx-auto flex w-full max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-5 sm:gap-4 sm:px-6 sm:py-6 lg:px-8">
        <Link
          href="/"
          className="flex items-center gap-3 text-sm font-semibold uppercase tracking-[0.24em] text-[var(--text-secondary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
        >
          <span className="grid h-9 w-9 place-items-center rounded-2xl border border-[var(--border-color)] bg-[var(--surface-strong)] text-base text-[var(--ink)] shadow-[0_0_0_1px_rgba(255,255,255,0.03)]">
            M
          </span>
          MindShift
        </Link>
        ...
        <Link
          href="/sign-in"
          className="inline-flex min-h-11 items-center rounded-full border border-[var(--border-color)] bg-[var(--surface-strong)] px-4 py-2 text-sm font-medium text-[var(--ink)]/78 transition-colors hover:bg-[var(--surface-strong)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
        >
          {t.signIn}
        </Link>
      </header>
      ...
      <section className="relative z-10 mx-auto grid w-full max-w-7xl gap-8 px-4 pb-12 pt-4 sm:px-6 lg:grid-cols-[1.08fr_0.92fr] lg:gap-12 lg:px-8 lg:pb-16 lg:pt-10">
        <div className="flex min-w-0 flex-col justify-center space-y-7">
          <div className="inline-flex w-fit items-center gap-2 rounded-full border border-[var(--border-color)] bg-[var(--surface-strong)] px-4 py-2 text-xs font-medium uppercase tracking-[0.2em] text-warning">
          ...
          <div className="min-w-0 space-y-5">
            ...
            <p className="max-w-2xl text-base leading-7 text-[var(--ink)]/72 sm:text-lg">{t.subtitle}</p>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-stretch">
            <div className="flex flex-col gap-1.5">
              <Link
                href="/sign-up"
                className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-primary px-6 text-base font-semibold text-[var(--ink)] shadow-[0_0_40px_rgba(139,92,246,0.35)] transition-transform [@media(hover:hover)]:hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
              >
                {t.parentCta}
                <ArrowRight className="h-5 w-5" aria-hidden="true" />
              </Link>
              <span className="text-center text-xs text-[var(--ink)]/52">{t.parentCtaHint}</span>
            </div>

            <div className="flex flex-col gap-1.5">
              <Link
                href="/enter-code"
                className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl border border-[var(--border-color)] bg-[var(--surface-strong)] px-6 text-base font-semibold text-[var(--ink)]/88 transition-colors hover:bg-[var(--surface-strong)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60 focus-visible:ring-offset-2 focus-visible:ring-offset-background"
              >
                <KeyRound className="h-5 w-5 text-warning" aria-hidden="true" />
                {t.childCta}
              </Link>
              <span className="text-center text-xs text-[var(--ink)]/52">{t.childCtaHint}</span>
            </div>
          </div>
          ...
          <p className="text-sm text-[var(--text-muted)]">
            ...
            <Link
              href="/request-access"
              className="font-medium text-[var(--color-secondary-dark)] underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60"
            >
              {t.requestCta}
            </Link>
          </p>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {t.benefits.map((pillar) => {
              ...
              return (
                <article
                  key={pillar.title}
                  className="rounded-[18px] border border-[var(--border-color)] bg-surface/90 p-4 shadow-[0_18px_60px_rgba(0,0,0,0.16)]"
                >
                  <span className="mb-3 grid h-10 w-10 place-items-center rounded-2xl border border-[var(--border-color)] bg-[var(--surface-strong)]">
                    <Icon className="h-5 w-5 text-violet-400" aria-hidden="true" />
                  </span>
                  <h2 className="text-sm font-semibold text-[var(--ink)]">{pillar.title}</h2>
                  <p className="mt-1.5 text-xs leading-5 text-[var(--ink)]/62">{pillar.copy}</p>
                </article>
              );
            })}
          </div>
        ...
        <aside
          ...
          className="self-center rounded-[32px] border border-[var(--border-color)] bg-surface/90 p-6 shadow-[0_30px_120px_rgba(0,0,0,0.28)] sm:p-7"
        >
          <p className="text-xs font-medium uppercase tracking-[0.24em] text-warning">Старт без путаницы</p>
          <h2 id="family-start-title" className="mt-3 text-3xl font-semibold tracking-tight text-[var(--ink)]">
            {t.journeyTitle}
          </h2>
          <ol className="mt-6 space-y-4">
            {t.journey.map((step, index) => {
              ...
              return (
                <li key={step.title} className="flex gap-4 rounded-2xl border border-[var(--border-color)] bg-[var(--surface-strong)] p-4">
                  <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-primary/15 text-sm font-semibold text-[var(--color-secondary-dark)]">
                    {index + 1}
                  </span>
                  <div>
                    ...
                    <p className="mt-1.5 text-sm leading-6 text-[var(--ink)]/62">{step.copy}</p>
                  </div>
                </li>
              );
            })}
          </ol>
          <div className="mt-6 flex items-start gap-3 rounded-2xl border border-primary/20 bg-primary/[0.08] p-4 text-sm leading-6 text-[var(--ink)]/76">
            <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-[var(--color-secondary-dark)]" aria-hidden="true" />
      ...
      <section aria-labelledby="try-title" className="relative z-10 mx-auto w-full max-w-5xl px-6 py-12 lg:px-8 lg:py-16">
        <div className="mx-auto mb-7 max-w-2xl text-center">
          ...
          <h2 id="try-title" className="mt-3 text-balance text-3xl font-semibold text-[var(--ink)] sm:text-4xl">
            {t.tryTitle}
          </h2>
          <p className="mt-3 text-sm leading-6 text-[var(--ink)]/64 sm:text-base">{t.tryCopy}</p>
        </div>
        <InteractiveShowcase />
      </section>

      <section className="relative z-10 mx-auto w-full max-w-7xl px-6 pb-16 lg:px-8 lg:pb-24">
        <h2 className="mb-8 text-center text-2xl font-semibold text-[var(--ink)]">{t.howTitle}</h2>
        <div className="grid gap-4 md:grid-cols-3">
          {t.steps.map((step) => (
            <article key={step.tag} className="rounded-[20px] border border-[var(--border-color)] bg-surface/70 p-6">
              <p className="text-sm font-medium uppercase tracking-[0.2em] text-[var(--ink)]/45">{step.tag}</p>
              <h3 className="mt-3 text-lg font-semibold text-[var(--ink)]">{step.title}</h3>
              <p className="mt-2 text-sm leading-6 text-[var(--ink)]/62">{step.copy}</p>
            </article>
          ))}
        </div>
      </section>
