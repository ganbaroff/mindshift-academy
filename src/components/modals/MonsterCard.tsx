  ...
  return (
    <div className="fixed inset-0 z-[1100] bg-black/95 flex items-center justify-center p-4 overflow-y-auto overscroll-contain animate-in fade-in duration-500 motion-reduce:animate-none">
      <div
        ...
        tabIndex={-1}
        className="bg-[#0d1527] border border-[var(--border-color)] rounded-3xl p-6 md:p-8 max-w-[480px] w-full text-center relative shadow-[0_0_50px_rgba(139,92,246,0.2)] outline-none"
      >
        ...
        <div className="border border-[var(--border-color)] bg-[var(--surface-strong)] rounded-2xl p-4 flex flex-col gap-4 relative overflow-hidden group">
          <div 
          ...
          {/* Monster Image */}
          <div className="aspect-square w-full rounded-xl overflow-hidden bg-black/40 border border-[var(--border-color)] relative">
          ...
          <div className="flex justify-between items-center z-10 px-1">
            <div className="text-left">
              <h2 id="monster-card-title" className="font-extrabold text-xl text-[var(--ink)]">{generatedMonster.name}</h2>
          ...
          {/* Stats visualization */}
          <div className="grid grid-cols-3 gap-2 bg-[var(--surface-strong)] border border-[var(--border-color)] rounded-xl p-3 z-10 text-xs">
            <div className="flex flex-col gap-0.5">
              <span className="text-[var(--text-muted)] text-[10px] uppercase">Сила</span>
              <span className="font-extrabold text-amber-500">85 / 100</span>
            </div>
            <div className="flex flex-col gap-0.5">
              <span className="text-[var(--text-muted)] text-[10px] uppercase">Интеллект</span>
              <span className="font-extrabold text-cyan-400">98 / 100</span>
            </div>
            <div className="flex flex-col gap-0.5">
              <span className="text-[var(--text-muted)] text-[10px] uppercase">Логика</span>
        ...
        <div className="flex flex-col gap-3 mt-6">
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 bg-[var(--surface-strong)] hover:bg-[var(--surface-strong)] text-[var(--ink)] font-extrabold py-3 border border-[var(--border-color)] rounded-full text-sm [@media(hover:hover)]:hover:scale-[1.02] transition-[transform,background-color] duration-[160ms] [transition-timing-function:var(--ease-out)] cursor-pointer"
            >
              <span aria-hidden="true">🔗</span> Поделиться
            </button>
          </div>

          <p aria-live="polite" className="min-h-[1rem] text-xs text-[var(--color-primary-dark)]">
            {copyNotice}
          </p>

          <button
            onClick={handleResetApp}
            className="self-center text-xs font-semibold text-[var(--text-muted)] hover:text-[var(--text-secondary)] underline underline-offset-2 transition-colors cursor-pointer mt-1"
          >
