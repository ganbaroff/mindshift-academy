    ...
    >
      ...
      <div className="relative z-10 w-full h-full flex items-center justify-center">
        {/* One shot, never a loop. The face used to run `loop` forever for anyone without
            ...
            Safe to stop looping: all four faces are generated by
            scripts/build-monster-faces.mjs, whose assertRest() proves the `"cm": "rest"`
            marker sits on the last frame and that every animated property is motionless
            there, so the frozen last frame is a readable pose by construction, not by
            comment. `key={mood}` remounts the player when the mood changes, because a
            one-shot animation would otherwise stay frozen on the previous mood's last
            frame and the monster would never visibly react again. */}
        <Lottie
          key={mood}
